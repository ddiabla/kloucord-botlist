...
module.exports = class extends Command {
    ...
    async run(message, [user]) {
        if (!user || !user.bot) return message.channel.send(`Ese bot... Apostaria por que no existe :thinking:`);
        if (user.id === message.client.user.id) return message.channel.send(`-_- No`);

        const bot = await Bots.findOne({ botid: user.id }, { _id: false })
        if (!bot) return message.channel.send(`No encuentro ese bot :S`);
        let servers;
        ...
        let e = new MessageEmbed()
            ...
            e.addField(`Prefix`, bot.prefix ? bot.prefix : "Failed To Check Prefix", true)

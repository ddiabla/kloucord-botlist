...
route.post("/:id", auth, async (req, res) => {
    ...
    if (original && original.state !== "deleted")
        return res.json({ success: false, message: "¡Oh! Parece que tu bot ya está con nosotros.", button: { text: "¿Editar?", url: `/bots/edit/${bot.id}` } });
    ...
    try {
        await req.app.get('client').channels.cache.find(c => c.id === server.mod_log_id).send(`<:db_add:871169292551458879> - <@${req.user.id}> ha ${resubmit ? "RE-" : ""}subido su bot **<@${req.params.id}>**: <@&${server.role_ids.bot_verifier}>`);
        return res.json({ success: true, message: "¡Bien! Tu bot ha sido añadido, deberás esperar a que sea revisado" })
    } catch (e) {
        return res.json({ success: true, message: "¡Bien! Tu bot ha sido añadido, deberás esperar a que sea revisado" })
    }
});
